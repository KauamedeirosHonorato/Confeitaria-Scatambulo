(function () {
  ...
  function initModals() {
    ...
    if (closeTutorialModal)
      closeTutorialModal.addEventListener("click", closeTutorial);

    const closeTutorialFooterBtn = document.getElementById("close-tutorial-modal-footer");
    if (closeTutorialFooterBtn) {
  ...
  function showToast(message, type = 'success') {
      ...
      toast.innerHTML = `
          ...
          <div class="toast-content flex-grow flex flex-col gap-0.5 min-w-[200px] opacity-0 transition-opacity duration-300 delay-100 pl-3 pr-6">
              <span class="font-playfair font-bold text-gray-800 text-base">${config.title}</span>
              <span class="text-xs text-gray-600 font-medium leading-tight truncate w-full block">${message}</span>
              ${isSuccess ? `<button onclick="openCartPanelAndCloseToast(this)" class="text-left text-xs font-bold ${config.btnColor} hover:underline focus:outline-none">
                  Ver carrinho
              </button>` : ''}
          </div>
          <button class="close-toast-btn absolute top-2 right-2 text-gray-400 hover:text-gray-800 p-1 transition-colors opacity-0 duration-300">
  ...
  function updateCart() {
    ...
    if (cart.length > 0) {
      ...
      // Remove a classe de animação para poder reativá-la
      floatingCart.classList.remove("animate-cart-pulse");
      void floatingCart.offsetWidth;
      floatingCart.classList.add("animate-cart-pulse");
    } else {
      cartItemsPanel.innerHTML =
  ...
  function initCheckoutModal() {
    ...
    // --- FUNÇÕES AUXILIARES DE DATA/HORA (Movidus para o topo para acesso global no modal) ---
    const isToday = () => {
    ...
    const labelHorario = document.getElementById("label-horario");
    ...
    function handleRetiradaChange() {
      ...
      updateCheckoutTotal();
      updateTimeInputsState(); // Força o ajuste de horário ao mudar para retirada
    }
    ...
    checkoutOpenButtons.forEach((btn) => {
    ...
    const valorEntregaInput = valorEntregaSelect; // Reutiliza a variável já definida
    ...
    if (form) {
      form.addEventListener("submit", (e) => {
        ...
        const nfceCompany = document.getElementById('nfce-company-name');
